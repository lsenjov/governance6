 ...
 *
 * Covers `formatTimerValue` (MM:SS / -MM:SS, two-digit padding,
 * negative cap at -99:59) and `deriveTimerState` (boundary at
 * `now === dueAt`, the three persisted kinds). The component's tick
 * wiring and click semantics are covered by the broader behavioural
 * stack; these tests pin the math.
 *
...
describe("formatTimerValue", () => {
  it("formats positive remainders as MM:SS with zero padding", () => {
    ...
    expect(formatTimerValue(1_000)).toBe("00:01");
    expect(formatTimerValue(59_000)).toBe("00:59");
    expect(formatTimerValue(60_000)).toBe("01:00");
    expect(formatTimerValue(125_000)).toBe("02:05");
    expect(formatTimerValue(30 * 60_000)).toBe("30:00");
  });
  ...
  it("floors sub-second positives toward zero", () => {
    ...
    expect(formatTimerValue(999)).toBe("00:00");
  });

  it("formats negative remainders with a leading minus sign", () => {
    expect(formatTimerValue(-1_000)).toBe("-00:01");
    expect(formatTimerValue(-59_000)).toBe("-00:59");
    expect(formatTimerValue(-60_000)).toBe("-01:00");
    expect(formatTimerValue(-125_000)).toBe("-02:05");
  ...
  it("caps negative output at -99:59", () => {
    ...
    expect(formatTimerValue(capMs)).toBe("-99:59");
    // Past the cap — should clamp, not roll over.
    expect(formatTimerValue(capMs - 60_000)).toBe("-99:59");
    expect(formatTimerValue(-1_000_000_000)).toBe("-99:59");
  });
